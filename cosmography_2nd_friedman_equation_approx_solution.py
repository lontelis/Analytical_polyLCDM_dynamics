...
def inner_integrand(a):
    return 3 * Omega_m0 * a**-3 + 4 * Omega_r0 * a**-4
...
t_values_2nd_Friedman_eqn_approx = np.array(integral_values) / H0  # in Hubble time units

# Plot the results
plt.ion
plt.figure(4,figsize=(10, 6))
plt.plot(a_values, t_values_2nd_Friedman_eqn_approx)
plt.xscale('log')
plt.yscale('log')
plt.xlabel('Scale factor (a)')
#plt.ylabel('Time (in Hubble time units)')
plt.ylabel('Time (in sec)')
plt.title('Cosmic Time vs Scale Factor Approximate')
plt.grid(True)
plt.show()
